...
import { TripMessageData, TripAssignmentData, TripServiceType, TripFormData } from "@/components/trips/types";
...
export default function Trips() {
  ...
  const [serviceType, setServiceType] = useState<TripServiceType>("airport_pickup");
  ...
  const createRecurringTrips = async (formData: TripFormData, occurrences: number, frequency: "daily" | "weekly" | "monthly") => {
    const trips = [];
    
    for (let i = 0; i < occurrences; i++) {
      let tripDate = new Date(formData.date);
      ...
      trips.push({
        ...
        is_recurring: true,
        flight_number: formData.flight_details?.flight_number,
        airline: formData.flight_details?.airline,
        terminal: formData.flight_details?.terminal,
  ...
  const handleSaveTrip = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const serviceType = formData.get("service_type") as TripServiceType;
    ...
    const tripData: TripFormData = {
      ...
      dropoff_location: formData.get("dropoff_location") as string || undefined,
      is_recurring: isRecurring,
      special_notes: formData.get("special_notes") as string || undefined,
    };
    
    // Add return time for certain service types
    if (["round_trip", "security_escort", "full_day_hire"].includes(serviceType)) {
      tripData.return_time = formData.get("return_time") as string;
    }
    
    // Add flight details for airport pickups/dropoffs
    if (["airport_pickup", "airport_dropoff"].includes(serviceType)) {
      tripData.flight_details = {
    ...
    if (isRecurring) {
      tripData.recurrence_details = {
        start_date: formData.get("start_date") as string,
        end_date: formData.get("end_date") as string,
        frequency: formData.get("frequency") as "daily" | "weekly" | "monthly",
        occurrences: parseInt(formData.get("occurrences") as string) || 1,
      };
    }
      ...
      if (editTrip) {
        ...
        const { error } = await supabase
          ...
          .update({
            ...
            type: tripData.service_type,
            status: tripData.status,
            amount: tripData.amount,
            pickup_location: tripData.pickup_location,
            dropoff_location: tripData.dropoff_location,
            is_recurring: tripData.is_recurring,
            flight_number: tripData.flight_details?.flight_number,
            airline: tripData.flight_details?.airline,
            terminal: tripData.flight_details?.terminal,
            special_notes: tripData.special_notes,
          })
          .eq("id", editTrip.id);
        ...
        setEditTrip(null);
      } else if (isRecurring && tripData.recurrence_details) {
        // Create recurring trips
        const trips = await createRecurringTrips(
          tripData, 
          tripData.recurrence_details.occurrences, 
          tripData.recurrence_details.frequency
        );
                  ...
                  <TableRow key={trip.id} className="cursor-pointer hover:bg-muted/50" onClick={() => setViewTrip(trip)}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        {trip.is_recurring && <Repeat className="h-3 w-3 text-muted-foreground" />}
                      </div>
                    </TableCell>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    name="service_type" 
                    value={serviceType}
                    onValueChange={(value: TripServiceType) => setServiceType(value)}
